...
File: chatglmRequest.py
Description: Wrapper functions for chatglm.
"""
import json
import random
import time 
...
def Chatglm_request(prompt,model,tokenizer): 
  """
  Given a prompt and a dictionary of chatglm3 parameters, make a request and returns the response. 
  ARGS:
    prompt: a str prompt
    model: init_model() return model paremater
    tokenizer: init_model() return tokenizer method
  RETURNS: 
    a str of chatglm's response. 
  """
  # temp_sleep()
  try: 
...
"""
  respond = ""
  while(len(respond) < 100):
    respond , _ = Chatglm_request(prompt , model ,tokenizer)
  print(respond)
